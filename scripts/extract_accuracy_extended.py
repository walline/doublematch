...
def summary_dict(accuracies, steps):
    results = {'last%02d' % x: np.median(accuracies[-x:]) for x in [1, 10, 20, 50]}
    results["max"] = np.max(accuracies)
...
def main(argv):
    ...
    folder = argv[1]
    matches = sorted(tf.gfile.Glob(os.path.join(folder, 'tf/events.out.tfevents.*')))
    ...
    with tf.gfile.Open(target_file, 'w') as f:
        json.dump(summary_dict(accuracies, steps), f, sort_keys=True, indent=4)
