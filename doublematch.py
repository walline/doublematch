...
class DoubleMatch(CTAReMixMatch):
    ...
    def model(self, batch, lr, wd, wu, ws, confidence, uratio, cosinedecay, ema=0.999, **kwargs):
...
def main(argv):
    ...
    model = DoubleMatch(
        os.path.join(FLAGS.train_dir, dataset.name, DoubleMatch.cta_name()),
        dataset,
        lr=FLAGS.lr,
        wd=FLAGS.wd,
        arch=FLAGS.arch,
